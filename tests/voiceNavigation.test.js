...
// Audio-Mock: speichert welche Dateien abgespielt wurden
const playedAudios = [];
global.Audio = jest.fn().mockImplementation((src) => ({
    ...
    addEventListener: jest.fn((event, cb) => {
        if (event === 'ended') setTimeout(cb, 10); // sofort "ended" auslösen
    }),
}));

// SpeechSynthesis-Mock: speichert welche Texte gesprochen wurden
const spokenTexts = [];
global.window = {
    speechSynthesis: {
...
class VoiceNavigation {
    ...
    _playTTS(text) {
        if (!window.speechSynthesis) { this._playNext(); return; }
    ...
    _clearQueue() { this.queue = []; this.isPlaying = false; window.speechSynthesis?.cancel(); }
}
...
describe('T10 – Sprachliche Weganweisungen', () => {
    ...
    describe('Vorankündigung bei 30m', () => {
        ...
        test('spricht Straßenname auf Deutsch (de-AT)', () => {
            ...
            const sprechAufruf = window.speechSynthesis.speak.mock.calls[0][0];
